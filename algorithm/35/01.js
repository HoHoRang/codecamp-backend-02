...
function solution(n, words) {
  const prevWord = []; // 이전에 나왔던 단어들
  let lastChar = words[0][0]; // 가장 마지막 알파벳
  const result = [0, 0]; // 결과

  for (let i = 0; i < words.length; i++) {
    if (prevWord.includes(words[i]) || lastChar !== words[i][0]) {
      const who = (i % n) + 1;
...
//     if (prevWord !== currentWord || words.indexOf(words[i]) !== i) {
//       return [player, turn];
//     }
//   }
//   return [0, 0];
