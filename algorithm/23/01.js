...
function solution(N, stages) {
  ...
  for (let i = 1; i <= N; i++) {
    let arrival = 0; // 도달한 사람수(해당 스테이지 숫자보다 크거나 같은 사람)
    let fail = 0; // 실패한 사람수(해당 스테이지 숫자인 사람)

    arrival = stages.filter((ele) => ele >= i).length;
    fail = stages.filter((ele) => ele === i).length;

    const failRate = fail / arrival;
    const bbb = [i, failRate];

    tempArr.push(bbb);
    // [ [1, 0.125], [2, 0.4], ... ]
  }

  tempArr.sort((a, b) => (a[1] > b[1] ? -1 : 1));
